...
`192.168.2.243`.** Manifests are written, schema-validated offline, and
...
kubectl --context admin@contraxia -n workspaces get svc devbox-ssh   # EXTERNAL-IP 192.168.2.243
...
**`192.168.2.243`** from `lan-pool`, pinned with
...
| The `192.168.2.243` address | yes | yes | yes |
...
OK    devbox-ssh: LB IP 192.168.2.243, selector {'vm.kubevirt.io/name': 'devbox'}
...
finish → merge → watch `get dv` then `get vmi` → `ssh ubuntu@192.168.2.243`.
...
**VIP reassigned `192.168.2.241` → `192.168.2.243`.** Three tracks ran in
parallel and each independently reasoned ".240 is taken, so .241 is first free";
all three claimed `.241`. The Orca cell (Track A) kept `.241` because its
address is baked into `PAIRING_ADDRESS` and the Mac's pairing instructions.
Gitea took `.242`, this devbox took `.243`. The authoritative map is
`docs/vip-allocation.md`; every occurrence in this report and in the
`cpu-workspace` manifests was rewritten to `.243` by the coordinator.
