...
|---|---|---|
| `01-cilium.sh` | Cilium (CNI + Hubble) + LAN LB (`cilium-lb-lan.yaml`: LB-IPAM `lan-pool` + L2 announce) | Cluster has `cni.name=none` — no pod schedules until Cilium is up |
...
`cilium-lb-lan.yaml` (LB-IPAM pool 192.168.2.240–250 + L2AnnouncementPolicy)
is applied by `01-cilium.sh` — the LAN LB path for the arrakis CP endpoint.
Sveltos install
